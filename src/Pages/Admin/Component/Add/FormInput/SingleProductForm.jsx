...
export default function SingleProductForm(props) {
  const [cate, setcate] = useState([]);
  const [name, setName] = useState(props.single.name);
  const [price, setPrice] = useState(props.single.price);
  const [qty, setQty] = useState(props.single.qty);
  const [discount, setDiscount] = useState(props.single.discount);
  const [time, setTime] = useState(props.single.time);
  const [cat, setCat] = useState(props.single.categoryId.name);
  const [avatar, setAvatar] = useState(props.single.avatar);
  const [imgs, setImgs] = useState(props.single.imgs.map((itm, index) => itm));
  const [detail, setdetail] = useState(props.single.detail);
  const [ingredient, setIngredient] = useState(props.single.ingredient);
  const [other, setOther] = useState(props.single.other);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `https://restaurantmanagement-h0y1.onrender.com/api/product/${props.id}`,
        form,
        {
          withCredentials: true,
      ...
      if (data) {
        return (
          props.setProducts(data.saveProduct),
          setIsFetching(false),
  ...
  return (
    <>
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        <div className="modalItem">
          <label htmlFor="others">Other Details</label>
          <textarea
            id=""
            value={other}
            name="other"
